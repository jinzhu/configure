...
syn keyword zimbuCompType	string
...
syn keyword zimbuStatement IF ELSE ELSEIF WHILE FOR IN DO UNTIL SWITCH
...
syn keyword zimbuStatement NEW RETURN EXIT EQUAL
syn keyword zimbuStatement IMPORT MAIN
syn keyword zimbuStatement INTERFACE MODULE ENUM BITS SHARED
syn match zimbuStatement "\<\(FUNC\|PROC\)\(\.default\|\.abstract\)\=\>"
syn match zimbuStatement "\<CLASS\(\.final\|\.abstract\)\=\>"
syn match zimbuStatement "}"

syn keyword zimbuOperator IS ISNOT ISA ISNOTA

syn keyword zimbuModule  IO SYS ARG HTTP ZWT
