...
" http://maxiangjiang.googlepages.com/vimim.pinyin_5MB.txt
...
" http://maxiangjiang.googlepages.com/vimim.nature.txt
...
" ------------------------------------------
    ...
    call add(input_methods, "pinyin_5MB")
...
" ---------------------------
    ...
    if s:onekey_loaded_flag < 1
        ...
        call s:vimim_punctuation_hjkl_on()
        call s:vimim_static_hjkl_on()
        if s:vimim_tab_for_one_key > 0
        inoremap<silent><Space> <C-R>=<SID>vimim_dynamic_space()<CR>
        endif
    endif
    let onekey = ""
    if pumvisible()
...
" -------------------------------------
function! s:vimim_punctuation_hjkl_on()
" -------------------------------------
    if s:vimim_menu_hjkl < 1
        return
    endif
    " ------------------------------------
    " punctuation navigation for all modes
    " ------------------------------------
    let hjkl_list = split(',.=-;?/[]', '\zs')
    for _ in hjkl_list
        sil!exe 'inoremap<silent><expr> '._.'
        \ <SID>vimim_punctuation_hjkl("'._.'")'
    endfor
endfunction

" --------------------------------
function! s:vimim_static_hjkl_on()
" --------------------------------
    if s:vimim_menu_hjkl < 1
        return
    endif
    ...
    " -------------------------------------
    if s:chinese_input_mode < 2
        let hjkl_list = split('hjkleryc', '\zs')
        for _ in hjkl_list
            sil!exe 'inoremap<silent><expr> '._.'
            \ <SID>vimim_hjkl("'._.'")'
        endfor
...
" ----------------------------------------
function! <SID>vimim_punctuation_hjkl(key)
" ----------------------------------------
    let hjkl = a:key
    if pumvisible()
        if a:key == ";"
            let hjkl = '\<C-E>\<C-X>\<C-U>\<Down>\<C-Y>'
        elseif a:key == "["
        ...
        if s:chinese_input_mode > 0
        \&& s:vimim_chinese_punctuation > 0
        \&& has_key(s:punctuations, hjkl)
            let hjkl = s:punctuations[hjkl]
        endif
    endif
...
" ----------------------------
    ...
    if pumvisible()
        ...
        elseif a:key == 'l'
            let hjkl = '\<PageDown>'
        elseif a:key == 'c'
            let hjkl = '\<C-Y>\<C-R>=g:vimim_copy_popup_chinese()\<CR>'
        elseif a:key == 'r'
...
" ------------------------------------
    ...
    if has("gui_running")
        let @+ = chinese
    elseif s:vimim_auto_copy_clipboard > 0
        let @0 = chinese
    endif
    return "\<Esc>"
endfunction
...
" ---------------------------
    if s:vimim_static_input_style > 0
        ...
        " ----------------------------
        sil!call s:vimim_punctuation_hjkl_on()
        sil!call s:vimim_static_hjkl_on()
        " ----------------------------
    else
...
" ------------------------------------------------
    ...
    " --------------------------------------
    sil!call <SID>vimim_punctuation_mapping()
...
" -----------------------------------------
    ...
    let char_first = line_no_leading_space[0]
    let char_second = line_no_leading_space[1]
    if char_first =~ '[/?]'
    \&& char2nr(char_second) > 127
        let char = char_first
    endif
    return char
endfunction
...
" --------------------------
    let char = s:vimim_first_char_current_line()
    " --------------------------------------------
...
" -------------------------------------------------
function! s:vimim_slash_popup_chinese(column_start)
" -------------------------------------------------
    let chinese = s:vimim_popup_chinese(a:column_start)
    if empty(chinese)
        let @/ = @_
        let chinese = "\<Esc>"
    else
...
" ------------------------------
    ...
    if empty(slash)
        ...
        let chinese = s:vimim_slash_popup_chinese(column_start)
        let repeat_times = len(chinese)/s:multibyte
...
" ------------------------------------
    ...
    if pumvisible()
        if len(s:keyboard_sentence) > 0
            let s:bs = 1
            let key = s:vimim_keyboard_by_word()
        endif
...
" ---------------------------------------
    let s:vimim_chinese_punctuation=(s:vimim_chinese_punctuation+1)%2
    return <SID>vimim_punctuation_mapping()
endfunction

" ----------------------------------------
function! <SID>vimim_punctuation_mapping()
" ----------------------------------------
    for key in keys(s:punctuations)
        let value = key
        if s:vimim_chinese_punctuation > 0
        \&& has_key(s:punctuations, key)
...
" ----------------------------------------------
    ...
    """ step 4/4: insert the new order list into memory
...
" -------------------------------------------------------
    ...
    for key in keys(a:hash_0)
        ...
        if empty(a:hash_2)
        \&& has_key(a:hash_1, char_first)         |" ml7712
            let menu_vary = a:hash_0[key]         |" ml
            let menu_fix  = a:hash_1[char_first]  |" 7712
            let menu = menu_fix.space.menu_vary   |" 7712 ml
            let pair = menu . " " . key           |" 7712 ml 马力
        ...
        \&& has_key(a:hash_2, char_last)
            ...
            let menu = menu_fix1.space.menu_fix2.space.menu_vary
            let pair = menu . " " . key
            call add(values, pair)
        endif
    endfor
