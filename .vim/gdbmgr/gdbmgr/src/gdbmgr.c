...
Dprintf((fp,"gdbmgr=%px gdbcmd<%s>%s\n",
  gdbmgr,
  gdbcmd? gdbcmd : "-null-",
...
if(gdbmgr->init == 2) {
	...
	/* look for prompt */
	shortwaitcnt= 0;
	Dprintf((fp,"(gmGdb) look for prompt\n"));
	while(1) {
		if(!fdgets(buf,MEDBUFSIZE,gdbmgr->rwfd)) {
			if(++shortwaitcnt > SHORTWAITCNTMAX) {
				Dprintf((fp,"(looking for prompt) shortwaitcnt>%d!\n",SHORTWAITCNTMAX));
    	...
    	Dprintf((fp,"(gmGdb init) buf<%s>\n",buf));
        if(!strcmp(buf,"\x1a\x1aprompt")) {        /* the annotate=3 prompt string     */
			Dprintf((fp,"[prompt] encountered\n"));
...
gdbmgr->mlbuf[1] = '\0';
Dprintf((fp,"mlbuf initialized to empty\n"));
...
if(gdbmgr->running == RUNNING || gdbmgr->running == QUERY) {
	char *ret;
...
Dprintf((fp,"(gmGdb) look for <c-z><c-z>post-prompt\n"));
shortwaitcnt= 0;
while(1) {
	if(!fdgets(buf,MEDBUFSIZE,gdbmgr->rwfd)) {
		if(++shortwaitcnt > SHORTWAITCNTMAX) {
			Dprintf((fp,"(looking for post-prompt) shortwaitcnt>%d!\n",shortwaitcnt));
			strcpy(gdbmgr->mlbuf,"Sgdb not responding as expected");
			gdbmgr->running  = STOPPED;
			record2mlbuf     = 0;
			Rdbg((fp,"gmGdb <%s>",gdbmgr->mlbuf));
    ...
    Dprintf((fp,"(gmGdb) looking for post-prompt: buf<%s> gdbmgr->running=%d  (will attempt to recognize)\n",buf,gdbmgr->running));
	if(!strncmp(buf,"\x1a\x1apost-prompt",13)) {
		Dprintf((fp,"[post-prompt] encountered\n"));
		break;
		}
	else if(buf[0] ==  '\x1a' && buf[1] == '\x1a') { /* skip over annotations; modify running status if so indicated */
		Dprintf((fp,"(gmGdb) buf[0] and buf[1] both are <c-z>; skipping over most intermediate annotations\n"));
		if(!strncmp(buf+2,"exited ",7)) {
			gdbmgr->running  = STOPPED;
			gdbmgr->mlbuf[0] = 'S';
			...
			Dprintf((fp,"(gmGdb) [starting %d] encountered\n",exitstatus));
			continue;
			}
			...
			gdbmgr->mlbuf[0] = 'S';
			strcpy(gdbmgr->mlbuf+1,buf+2);
			Dprintf((fp,"(gmGdb) [stopped %d] encountered\n",exitstatus));
			continue;
			}
			...
			gdbmgr->mlbuf[0] = 'S';
			strcpy(gdbmgr->mlbuf+1,buf+2);
			Dprintf((fp,"(gmGdb) [signalled %d] encountered\n",exitstatus));
			continue;
			}
		else {
			Dprintf((fp,"(gmGdb) [%s] encountered\n",buf+2));
			continue;
			}
		strcpy(mlb,buf);
		mlb+= strlen(mlb);
	...
	Dprintf((fp,"(gmGdb) continue looking for <c-z><c-z>post-prompt\n"));
	}

/* look for pre-prompt, filling in mlbuf with gdb's response to the command,
 * except for any intervening annotations.
 */
Dprintf((fp,"(gmGdb) look for pre-prompt, %s\n",
  record2mlbuf? "filling in mlbuf with gdb's response" : "mlbuf recording disabled"));
shortwaitcnt= 0;
while(1) {
	if(!fdgets(buf,MEDBUFSIZE,gdbmgr->rwfd)) {
    ...
    Dprintf((fp,"(gmGdb) looking for pre-prompt: buf<%s>\n",buf));
	if(buf[0] == '\x1a' && buf[1] == '\x1a') {
		/* buffer begins with <c-z><c-z> */
		if(!strncmp(buf+2,"pre-prompt",10)) {       /* <c-z><c-z>pre-prompt  */
			Dprintf((fp,"(gmGdb) looking for pre-prompt: encountered\n"));
			break;
			}
		else if(!strncmp(buf+2,"error-begin",11)) { /* <c-z><c-z>error-begin */
			Dprintf((fp,"(gmGdb) looking for pre-prompt: [error-begin] encountered\n"));
			continue;
			}
		else if(!strncmp(buf+2,"error",5)) {        /* <c-z><c-z>error       */
			Dprintf((fp,"(gmGdb) looking for pre-prompt: [error] encountered\n"));
			Rdbg((fp,"gmGdb <%s>",gdbmgr->mlbuf));
			return gdbmgr->mlbuf;
			}
			...
			gdbmgr->mlbuf[0] = 'S';
			sscanf(buf+2,"%*s %d",&exitstatus);
			Dprintf((fp,"(gmGdb) looking for pre-prompt: [exited %d] encountered\n",exitstatus));
			continue;
			}
			...
			gdbmgr->mlbuf[0] = 'S';
			strcpy(gdbmgr->mlbuf+1,buf+2);
			Dprintf((fp,"(gmGdb) looking for pre-prompt: [stopped] encountered\n"));
			break;
			}
			...
			gdbmgr->mlbuf[0] = 'S';
			strcpy(gdbmgr->mlbuf+1,buf+2);
			Dprintf((fp,"(gmGdb) looking for pre-prompt: [signalled] encountered\n"));
			break;
			}
		else if(!strncmp(buf+2,"pre-query",9)) {    /* <c-z><c-z>pre-query   */
			strcpy(gdbmgr->mlbuf+1,buf+2);
			gdbmgr->mlbuf[0]= 'Q';
			Dprintf((fp,"(gmGdb) looking for <c-z><c-z>query: [pre-query] encountered\n"));
			continue;
			}
	...
	*mlb   = '\0';         /* insure that mlbuf has a properly terminated string */
	Dprintf((fp,"(gmGdb) mlbuf<%s>\n",gdbmgr->mlbuf));
	}

/* look for <c-z><c-z>prompt */
Dprintf((fp,"look for prompt\n"));
shortwaitcnt= 0;
while(1) {
	if(!fdgets(buf,MEDBUFSIZE,gdbmgr->rwfd)) {
		if(++shortwaitcnt > SHORTWAITCNTMAX) {
	...
	Dprintf((fp,"(prompt) buf<%s>\n",buf));
	if(!strncmp(buf,"\x1a\x1aprompt",8)) {
		Dprintf((fp,"[prompt] encountered\n"));
		break;
		}
	}
