...
" braces in comments.
let b:match_words = '\(^\s*\)\@<=\(MODULE\|CLASS\|BITS\|ENUM\|SHARED\|FUNC\|REPLACE\|DEFINE\|PROC\|NEW\|EQUAL\|MAIN\|IF\|GENERATE_IF\|WHILE\|DO\|FOR\|SWITCH\)\>:\(^\s*\)\@<=\(ELSE\|ELSEIF\|GENERATE_ELSE\|GENERATE_ELSEIF\)\>:\(^\s*\)\@<=}\|\<UNTIL\>'
